...
from continualworld.sac.replay_buffers import ReplayBuffer, ReservoirReplayBuffer, Episodicbuffer
...
class SAC:
    def __init__(
        ...
        transition_size_of_offline_dataset: int = 1_000_000,
        collect_offline_data: bool = False,
        save_path_of_offline_data: str = "offline_metaworld_data",
        task=None,
    ):
        ...
        self.transition_size_of_offline_dataset = transition_size_of_offline_dataset
        self.collect_offline_data = collect_offline_data
        self.save_path_of_offline_data = save_path_of_offline_data
        self.task = task
        self.before_expert_collection_ep = 0
        ...
        self.expert_replay_buffer = ReplayBuffer(
            obs_dim=self.obs_dim, act_dim=self.act_dim, size=replay_size, buffer_name="expert"
        )
    ...
    def save_model(self, current_task_idx):
        ...
        if current_task_idx == -1:
            dir_prefixes.append(f"./checkpoints/{self.task}")
        else:
            dir_prefixes.append(f"./checkpoints/{self.task}_task{current_task_idx}")
    ...
    def after_run_for_expert_data(self):
        success_rate, ep = [], 0
        obs, episode_return, episode_len = self.env.reset(), 0, 0
        print(colored("Collect expert data", color="green"))
        for global_timestep in range(int(self.transition_size_of_offline_dataset)):
            ...
            self.expert_replay_buffer.store(obs, action, reward, next_obs, done_to_store, timeout)
            ...
            if done or (episode_len == self.max_episode_len):
                ep += 1
                success_rate.append((self.env.pop_successes())[-1])
                if global_timestep < self.steps - 1:
                    obs = self.env.reset()
            if (global_timestep + 1) % self.log_every == 0:
                print(colored(f"Have collected [{ep}] expert trajectories and average success rate is [{np.mean(success_rate)}].", color="green"))
            # if (self.transition_size_of_offline_dataset - global_timestep) < self.max_episode_len:
            #     self.expert_replay_buffer.valid_buffer_clip()

    def run(self):
        """A method to run the SAC training, after the object has been created."""
        self.start_time = time.time()
        obs, episode_return, episode_len = self.env.reset(), 0, 0
        print(colored("Collect train data", color="green"))
        ...
        for global_timestep in range(self.steps):
            ...
            # Store experience to replay buffer
            self.replay_buffer.store(obs, action, reward, next_obs, done_to_store, timeout)
            ...
            # End of trajectory handling
            if done or (episode_len == self.max_episode_len):
                self.before_expert_collection_ep += 1
            ...
            if (current_task_timestep >= self.update_after and current_task_timestep % self.update_every == 0):
                for j in range(self.update_every):
                    ...
                    results = self.learn_on_batch(tf.convert_to_tensor(current_task_idx), batch, episodic_batch)
            ...
            if self.collect_offline_data:
                if (self.steps - current_task_timestep) < self.max_episode_len:
                    break

    def save_dataset(self, replay_buffer):
        replay_buffer.valid_buffer_clip()
        if not os.path.exists(self.save_path_of_offline_data):
            os.mkdir(self.save_path_of_offline_data)
        if not os.path.exists(os.path.join(self.save_path_of_offline_data, self.task)):
            os.mkdir(os.path.join(self.save_path_of_offline_data, self.task))
        offline_dataset = replay_buffer.get_offline_dataset()
        offline_dataset_name = "-".join([self.task, replay_buffer.buffer_name])
        offline_dataset_name = f"{offline_dataset_name}.hdf5"
        save_path = os.path.join(self.save_path_of_offline_data, self.task, offline_dataset_name)
        file = h5py.File(save_path, "w")
        # save HDF5 data
        for data_key, data_val in offline_dataset.items():
            file.create_dataset(data_key, data=data_val)
        # 关闭文件
        file.close()
