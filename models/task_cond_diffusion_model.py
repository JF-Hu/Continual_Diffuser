...
class TaskCondGaussianDiffusion(nn.Module):
    ...
    def p_losses(self, x_start, cond, t, task_identity, task_name, *args, **kwargs):
        noise = torch.randn_like(x_start)
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        x_noisy = self.apply_condition(x=x_noisy, cond=cond)
        if self.argus.unet_type in [UnetType.temporal_unet]:
            x_recon = self.model(x=x_noisy, cond=cond, timesteps=t, task_identity=task_identity, task_name=task_name)
        elif self.argus.unet_type in [UnetType.spatial_transformer_unet]:
            x_recon = self.model(x=x_noisy, cond=cond, timesteps=t, context=task_identity, task_name=task_name)
        else:
            raise NotImplementedError
    ...
    def loss(self, x, cond, task_identity, task_name, *args, **kwargs):
        self.model.train()
        info = {}
        batch_size = len(x)
        t = torch.randint(0, self.n_timesteps, (batch_size,), device=x.device).long()
        diffuse_loss, diffuse_loss_info = self.p_losses(x, cond, t, task_identity, task_name, *args, **kwargs)
    ...
    def get_action(self, cond, data_shape, task_identity, ddim_sample, task_name, *args, **kwargs):
        return self.conditional_sample(cond, data_shape, task_identity, ddim_sample, task_name=task_name, *args, **kwargs)
