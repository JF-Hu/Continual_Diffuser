...
class TaskReturnCondGaussianDiffusion(nn.Module):
    ...
    # @torch.no_grad()
    def p_mean_variance(self, x, cond, t, task_identity, task_name, returns, action_range):
        if self.argus.condition_type == DiffusionConditionType.classifier_free:
            epsilon_cond = self.model(x=x, cond=cond, timesteps=t, task_identity=task_identity, context=task_identity, task_name=task_name, returns=returns, use_dropout=False)
            epsilon_uncond = self.model(x=x, cond=cond, timesteps=t, task_identity=task_identity, context=task_identity, task_name=task_name, returns=returns, force_dropout=True)
            epsilon = epsilon_uncond + self.condition_guidance_w * (epsilon_cond - epsilon_uncond)
        elif self.argus.condition_type == DiffusionConditionType.unconditional:
            epsilon = self.model(x=x, cond=cond, timesteps=t, task_identity=task_identity, context=task_identity, task_name=task_name, returns=returns, use_dropout=False)
        else:
    ...
    @torch.no_grad()
    def p_sample(self, x, cond, t, task_identity, task_name, returns, action_range):
        b, *_, device = *x.shape, x.device
        model_mean, _, model_log_variance, x_recon = self.p_mean_variance(x=x, cond=cond, t=t, task_identity=task_identity, task_name=task_name, returns=returns, action_range=action_range)
    ...
    @torch.no_grad()
    def p_sample_loop(self, shape, cond, task_identity, task_name, returns, return_diffusion=False, ddim_sample=False, action_range=(-0.99999, 0.99999), *args, **kwargs):
        ...
        if ddim_sample:
            for i in reversed(range(0, self.ddim_sampler.generation_step)):
                ...
                if self.argus.condition_type == DiffusionConditionType.classifier_free:
                    epsilon_cond = self.model(x=x, cond=cond, timesteps=actual_t, task_identity=task_identity, context=task_identity, task_name=task_name, returns=returns, use_dropout=False)
                    epsilon_uncond = self.model(x=x, cond=cond, timesteps=actual_t, task_identity=task_identity, context=task_identity, task_name=task_name, returns=returns, force_dropout=True)
                    epsilon = epsilon_uncond + self.condition_guidance_w * (epsilon_cond - epsilon_uncond)
                elif self.argus.condition_type == DiffusionConditionType.unconditional:
                    epsilon = self.model(x=x, cond=cond, timesteps=actual_t, task_identity=task_identity, context=task_identity, task_name=task_name, returns=returns, use_dropout=False)
                else:
                    raise NotImplementedError
            ...
            for i in reversed(range(0, self.n_timesteps)):
                ...
                x = self.p_sample(x=x, cond=cond, t=timesteps, task_identity=task_identity, task_name=task_name, returns=returns, action_range=action_range)
                x = self.apply_condition(x=x, cond=cond)
                if return_diffusion: diffusion.append(x)
        if return_diffusion:
    ...
    def p_losses(self, x_start, cond, t, task_identity, task_name, returns, *args, **kwargs):
        ...
        x_recon = self.model(x=x_noisy, cond=cond, timesteps=t, context=task_identity, task_identity=task_identity, task_name=task_name, returns=returns)
    ...
    def loss(self, x, cond, task_identity, task_name, returns, *args, **kwargs):
        ...
        diffuse_loss, diffuse_loss_info = self.p_losses(x, cond, t, task_identity, task_name, returns, *args, **kwargs)
    ...
    def get_action(self, cond, data_shape, task_identity, ddim_sample, task_name, returns=0.95, *args, **kwargs):
        if isinstance(returns, float):
            returns = (torch.ones_like(task_identity) * returns).to(task_identity.device)
        return self.conditional_sample(cond, data_shape, task_identity, ddim_sample, task_name=task_name, returns=returns, *args, **kwargs)
