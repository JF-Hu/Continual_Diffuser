...
class LoRAResidualTemporalBlock(nn.Module):
    ...
    def __init__(self, inp_channels, out_channels, embed_dim, horizon, task_names, lora_dim, kernel_size=5, mish=True):
        ...
        self.blocks = nn.ModuleList([
            ...
            Conv1dLoRABlock(inp_channels=out_channels, out_channels=out_channels, kernel_size=kernel_size, mish=mish, task_names=task_names, lora_dim=lora_dim),
        ])
        ...
        self.time_act = act_fn
        self.time_mlp = LoRALinear(nn.Linear(embed_dim, out_channels), task_name=task_names, r=lora_dim)
    ...
    def forward(self, x, t, task_name):
        ...
        '''
        time_embed = self.time_mlp(self.time_act(t), task_name)
        ...
        out = self.blocks[1](out, task_name)
        return out + self.residual_conv(x, task_name)

class LoRATaskBasedTemporalUnet(nn.Module):
    ...
    def __init__(
        ...
        self.task_identity_mlp1 = LoRALinear(nn.Linear(1, dim), task_name=task_names, r=lora_dim)
        self.task_identity_act1 = act_fn
        self.task_identity_mlp2 = LoRALinear(nn.Linear(dim, dim * 4), task_name=task_names, r=lora_dim)
        self.task_identity_act2 = act_fn
        self.task_identity_mlp3 = LoRALinear(nn.Linear(dim * 4, dim), task_name=task_names, r=lora_dim)
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            self.downs.append(nn.ModuleList([
                LoRAResidualTemporalBlock(dim_in, dim_out, embed_dim=embed_dim, horizon=sequence_length, kernel_size=kernel_size, mish=mish, task_names=task_names, lora_dim=down_lora_dim) if ind!=-1 else ResidualTemporalBlock(dim_in, dim_out, embed_dim=embed_dim, horizon=sequence_length, kernel_size=kernel_size, mish=mish, task_names=task_names, lora_dim=down_lora_dim),
                LoRAResidualTemporalBlock(dim_out, dim_out, embed_dim=embed_dim, horizon=sequence_length, kernel_size=kernel_size, mish=mish, task_names=task_names, lora_dim=down_lora_dim) if ind!=-1 else ResidualTemporalBlock(dim_out, dim_out, embed_dim=embed_dim, horizon=sequence_length, kernel_size=kernel_size, mish=mish, task_names=task_names, lora_dim=down_lora_dim),
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out[1:])):
            ...
            self.ups.append(nn.ModuleList([
                LoRAResidualTemporalBlock(dim_out + downs_output, downs_input, embed_dim=embed_dim, horizon=sequence_length, kernel_size=kernel_size, mish=mish, task_names=task_names, lora_dim=lora_dim) if ind != -1 else ResidualTemporalBlock(dim_out + downs_output, downs_input, embed_dim=embed_dim, horizon=sequence_length, kernel_size=kernel_size, mish=mish, task_names=task_names, lora_dim=lora_dim),
                LoRAResidualTemporalBlock(downs_input, downs_input, embed_dim=embed_dim, horizon=sequence_length, kernel_size=kernel_size, mish=mish, task_names=task_names, lora_dim=lora_dim) if ind != -1 else ResidualTemporalBlock(downs_input, downs_input, embed_dim=embed_dim, horizon=sequence_length, kernel_size=kernel_size, mish=mish, task_names=task_names, lora_dim=lora_dim),
    ...
    def forward(self, x, cond, timesteps, task_name, task_identity, use_dropout=True, force_dropout=False, **kwargs):
        '''
        ...
        task_embed = self.task_identity_act1(self.task_identity_mlp1(task_identity, task_name))
        task_embed = self.task_identity_act2(self.task_identity_mlp2(task_embed, task_name))
        task_embed = self.task_identity_mlp3(task_embed, task_name)

        if use_dropout:
            mask = self.mask_dist.sample(sample_shape=(task_embed.size(0), 1)).to(task_embed.device)
            task_embed = mask * task_embed
        if force_dropout:
            task_embed = 0 * task_embed
        t = torch.cat([t, task_embed], dim=-1)
        h = []

        idx = 0
        for block_idx, (resnet, resnet2, downsample) in enumerate(self.downs):
            x = resnet(x, t, task_name) if block_idx != -1 else resnet(x, t)
            x = resnet2(x, t, task_name) if block_idx != -1 else resnet2(x, t)
            h.append(x)
            x = downsample(x, task_name)
            idx += 1
