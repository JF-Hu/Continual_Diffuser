...
class CL10(Benchmark):

    def __init__(self):
        super().__init__()
        self._train_classes = _env_dict.CL10_HARD_MODE_CLS_DICT['train'].copy()
        # We're going to modify it, so make a copy
        train_kwargs = _env_dict.CL10_HARD_MODE_ARGS_KWARGS['train'].copy()
        test_kwargs = _env_dict.CL10_HARD_MODE_ARGS_KWARGS['test']
        for (env_name, cls) in _env_dict.CL10_HARD_MODE_CLS_DICT['test'].items():
            assert env_name not in self._train_classes
            assert env_name not in train_kwargs
            self._train_classes[env_name] = cls
            train_kwargs[env_name] = test_kwargs[env_name]
